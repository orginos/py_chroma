...
from py_chroma import run_smd_xml


def main():
    parser = argparse.ArgumentParser(description="Run SMD via py_chroma.")
    parser.add_argument("input_xml", help="Path to SMD XML input")
    ...
    initialize()
    try:
        run_smd_xml(input_path.read_text())
    finally:
        finalize()
